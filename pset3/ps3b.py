...
class Patient(object):
    ...
    def update(self):
        ...
        total_viruses = [virus for virus in survivingViruses]
        for virus in survivingViruses:
            try:
                popDensity = len(total_viruses)/float(self.maxPop)
                total_viruses.append(virus.reproduce(popDensity))
            except NoChildException:
                pass
        self.viruses = total_viruses
        return self.viruses
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    """
    popSize = {i:[] for i in range(300)}
    simpleViruses = [SimpleVirus(maxBirthProb,clearProb)]*numViruses
    for trial in range(numTrials):
        patient = Patient(simpleViruses,maxPop)
        for i in range(300):
            patient.update()
            popSize[i].append(patient.getTotalPop())
    avgPopSize = [sum(pop)/float(len(pop)) for key,pop in popSize.items()]
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in activeDrugs:
            if not self.isResistantTo(drug):
                raise NoChildException()
        childResistances = {}
        for (drug,boolean) in self.resitances.items():
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        for virus in self.viruses:
            if len([drug for drug in drugResist if virus.isResistantTo(drug)]) == len(
                    drugResist):
                total += 1
        return total
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(numTrials):
        ...
        resistances.copy(), mutProb)] * numViruses
...
print (simulationWithDrug(1,1000,.1,0.05,{'guttagonol': False}, 0.005,1))
